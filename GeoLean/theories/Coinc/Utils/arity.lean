import GeoLean.theories.Axioms.beeson_s_axioms
import Init.Tactics
import Init.Data.Nat.Basic
/--
theorem minus_n_0 : forall n:Nat, Nat.sub n 0 = n :=
...
theorem minus_n_0 : forall n:Nat, n-0 = n := by
intro n
induction n; simp; rfl

theorem plus_0_n : forall n, 0+n = n := by
intro n
induction n with
  | zero =>
    rfl
...
theorem plus_n_0 : forall n, n+0 = n := by
intro n
induction n with
  | zero =>
    rfl
...
theorem plus_n_1 : forall n, n+1 = Nat.succ n := by
intro n
induction n with
  | zero =>
    rfl
...
theorem minus_n1_n2_0 : forall n1 n2:Nat, n1+n2-0 = n1+n2 := by
intro n1
induction n1 with
  | zero => intro n2; induction n2 with | zero =>  simp | succ k2 IHk2 => simp
  | succ k1 ih => intro n2; induction n2 with | zero =>  simp | succ k2 IHk2 => simp

def arity (T:Type) (n:Nat) :=
 match n with
 | 0 => Prop
 | Nat.succ p => T -> arity T p


/-- Warning: cartesianPower T 0 represents T but it is never used -/

def cartesianPowerAux (T:Type) (n:Nat) :=
 match n with
 | 0 => T
 | Nat.succ p => (T × cartesianPowerAux T p)

def cartesianPower T n := cartesianPowerAux T (n-1)

def headCP {T:Type} {n:Nat} (cp : cartesianPower T (Nat.succ n)) : T := by
induction n with
| zero => exact cp
| succ k ik => exact cp.fst

def headCPbis {T:Type} {n:Nat} (cp : cartesianPower T (Nat.succ n)) : cartesianPower T 1:= by
induction n with
| zero => exact cp
| succ k ik => exact cp.fst


def tailCP {T:Type} {n:Nat} (cp : cartesianPower T (Nat.succ (Nat.succ n))) : (cartesianPower T (Nat.succ n)) := by
induction n with
| zero => exact cp.snd
| succ k ik => exact cp.snd


def tailDefaultCP {T:Type} {n:Nat} (cp : cartesianPower T (Nat.succ n)) (Default : cartesianPower T n) : (cartesianPower T n) := by
induction n with
| zero => exact Default
| succ k ik => exact (tailCP cp)

/- def allButLastCP {T:Type} {n:Nat} (cp : cartesianPower T (Nat.succ (Nat.succ n))) : (cartesianPower T (Nat.succ n)) := by
induction n with
| zero => exact (headCP cp)
| succ k Hk =>
   constructor
   exact (headCP cp)
   unfold cartesianPower at cp
   simp at cp
   apply Hk
   exact (tailCP cp)


def allButLastCPTl {T:Type} {n:Nat} :
  forall (cp : cartesianPower T (Nat.succ (Nat.succ (Nat.succ n)))),
  allButLastCP (tailCP cp) = tailCP (allButLastCP cp) := by
intro cp; induction n; simp
