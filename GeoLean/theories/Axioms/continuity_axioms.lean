...
def segment_circle := forall A B P Q,
  InCircle P A B ->
  OutCircle Q A B ->
...
def one_point_line_circle := forall A B U V P,
...
def two_points_line_circle := forall A B U V P,
  Col U V P -> U ≠ V -> Bet A P B ->
...
def circle_circle := forall A B C D P Q:Tpoint,
  OnCircle P C D ->
  OnCircle Q C D ->
  InCircle P A B ->
  OutCircle Q A B ->
...
def circle_circle_bis := forall A B C D P Q:Tpoint,
  OnCircle P C D ->
  InCircle P A B ->
  OnCircle Q A B ->
  InCircle Q C D ->
...
def circle_circle_axiom := forall A B C D B' D':Tpoint,
...
def circle_circle_two := forall A B C D P Q:Tpoint,
  OnCircle P C D ->
  OnCircle Q C D ->
  InCircle P A B ->
  OutCircle Q A B ->
...
def euclid_s_prop_1_22 := forall A B C D E F A' B' C' D' E' F',
...
def Nested (A B:Nat -> Tpoint -> Prop) :=
  (forall n, exists An Bn, A n An /\ B n Bn) /\
  forall n An Am Bm Bn,
    A n An -> A (Nat.succ n) Am -> B (Nat.succ n) Bm -> B n Bn -> Bet An Am Bm /\ Bet Am Bm Bn /\ Am ≠ Bm

def cantor_s_axiom := forall A B, Nested A B ->
  exists X, forall n An Bn, A n An -> B n Bn -> Bet An X Bn

def dedekind_s_axiom := forall (Alpha Beta : Tpoint -> Prop),
  (exists A, forall X Y, Alpha X -> Beta Y -> Bet A X Y) ->
  (exists B, forall X Y, Alpha X -> Beta Y -> Bet X B Y)

def dedekind_variant := forall (Alpha Beta : Tpoint -> Prop) A C,
...
| forall_fof : forall P:Tpoint->Prop, (forall (A:Tpoint), FOF (P A)) -> FOF (forall A, P A)
| exists_fof : forall P:Tpoint->Prop, (forall (A:Tpoint), FOF (P A)) -> FOF (exists A, P A)

/-def first_order_dedekind := forall Alpha Beta:Tpoint->Prop,
  (forall X:Tpoint, FOF (Alpha X)) -> (forall Y:Tpoint, FOF (Beta Y)) ->
  (exists A, forall X Y, Alpha X -> Beta Y -> Bet A X Y) ->
  (exists B, forall X Y, Alpha X -> Beta Y -> Bet X B Y)-/

def archimedes_axiom := forall A B C D:Tpoint, A ≠ B -> Reach A B C D

def aristotle_s_axiom := forall P Q A B C:Tpoint,
  ¬ Col A B C -> Acute A B C ->
  exists X Y, Out B A X /\ Out B C Y /\ Per B X Y /\ Lt P Q X Y

def greenberg_s_axiom := forall P Q R A B C:Tpoint,
  ¬ Col A B C ->
...
def extension {Tm: Tarski_neutral_dimensionless} f := inj f /\ pres_bet f /\ pres_cong f


def completeness_for_planes := forall (Tm: Tarski_neutral_dimensionless)
...
def completeness_for_3d_spaces := forall (Tm: Tarski_neutral_dimensionless)
  (Tm2 : Tarski_neutral_dimensionless_with_decidable_point_equality Tm)
  (M : Tarski_3D Tm2)
  (f : @Tpoint Tn -> @Tpoint Tm),
  @archimedes_axiom Tm ->
...
def inj_line {T:Type} (f:Tpoint->T) P Q := forall A B, Col P Q A -> Col P Q B ->
  f A = f B -> A = B

def pres_bet_line {Tm: Tarski_neutral_dimensionless}
  (f : @Tpoint Tn -> @Tpoint Tm) P Q := forall A B C, Col P Q A -> Col P Q B -> Col P Q C ->
  Bet A B C -> Bet (f A) (f B) (f C)

def pres_cong_line {Tm: Tarski_neutral_dimensionless}
  (f : @Tpoint Tn -> @Tpoint Tm) P Q := forall A B C D,
  Col P Q A -> Col P Q B -> Col P Q C -> Col P Q D ->
  Cong A B C D -> Cong (f A) (f B) (f C) (f D)

def line_extension {Tm: Tarski_neutral_dimensionless} f P Q :=
  P <> Q /\ inj_line f P Q /\ pres_bet_line f P Q /\ pres_cong_line f P Q


def line_completeness := forall (Tm: Tarski_neutral_dimensionless)
  (Tm2 : Tarski_neutral_dimensionless_with_decidable_point_equality Tm)
  P Q
  (f : @Tpoint Tn -> @Tpoint Tm),
  @archimedes_axiom Tm ->
