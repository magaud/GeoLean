--import Coinc.Utils.general_tactics
...
class Tarski_neutral_dimensionless where
...
class Tarski_neutral_dimensionless_with_decidable_point_equality extends Tarski_neutral_dimensionless where
 point_equality_decidability : forall A B : Tpoint, A = B \/ ¬ A = B

class Tarski_2D extends Tarski_neutral_dimensionless_with_decidable_point_equality where
 upper_dim : forall A B C P Q,
   P ≠ Q -> Cong A P A Q -> Cong B P B Q -> Cong C P C Q ->
   (Bet A B C \/ Bet B C A \/ Bet C A B)

class Tarski_3D extends Tarski_neutral_dimensionless_with_decidable_point_equality where
 S1 : Tpoint
 S2 : Tpoint
 S3 : Tpoint
 S4 : Tpoint
...
class Tarski_euclidean extends Tarski_neutral_dimensionless_with_decidable_point_equality where
...
class Tarski_ruler_and_compass
 extends Tarski_neutral_dimensionless_with_decidable_point_equality where
 circle_circle_continuity : forall A B C D B' D',
   Cong A B' A B -> Cong C D' C D ->
   Bet A D' B -> Bet C B' D ->
   exists Z, Cong A Z A B /\ Cong C Z C D

class Tarski_continuous extends Tarski_neutral_dimensionless_with_decidable_point_equality where
