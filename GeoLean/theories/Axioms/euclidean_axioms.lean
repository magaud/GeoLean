...
class euclidean_neutral where
  ...
  axiom_connectivity :
   forall A B C D,
    BetS A B D -> BetS A C D -> ¬ BetS A B C -> ¬ BetS A C B -> eq B C
  axiom_nocollapse :
...
class euclidean_neutral_ruler_compass extends euclidean_neutral where
...
class euclidean_euclidean extends euclidean_neutral_ruler_compass where
  postulate_Euclid5 :
   forall a p q r s t,
...
class area extends euclidean_euclidean where
