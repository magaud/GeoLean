...
class Hilbert_neutral_dimensionless where
 ...
 two_points_on_line :
   forall l,
     ∃ (A : Point), ∃ (B : Point), IncidL B l /\ IncidL A l /\ A ≠ B
 ColH :=
...
class Hilbert_neutral_2D extends Hilbert_neutral_dimensionless where
 pasch_2D :
   forall A B C l,
     ¬ ColH A B C -> ¬ IncidL C l -> cut l A B ->
     cut l A C \/ cut l B C

class Hilbert_neutral_3D extends Hilbert_neutral_dimensionless where
 plane_intersection :
   forall A p q,
...
class Hilbert_euclidean extends Hilbert_neutral_dimensionless where
...
class Hilbert_euclidean_ID extends Hilbert_euclidean where
